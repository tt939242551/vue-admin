          ...
          <span>优惠券名称：</span>
           ...
           <span>数量限制：</span>
                  <i-input  style="width:160px" 
                     :number="true"   type="text" v-model="formDate[3]"  clearable placeholder="请输入限领次数"> </i-input>
      </div>
       <h5>优惠券设置</h5>
      <div class="main">
         <span>券有效期：</span>
                 <DatePicker  type="daterange"  @on-change="gettimes"  :value="formDate[4]" split-panels placeholder="请选择时间" style="width: 200px;margin-right: 50px;"></DatePicker>
          <span>优惠券类型：</span> 
               <Select v-model="formDate[5]" :disabled="index!=='add'" style="width:160px;margin-right: 15px;" placeholder="请选择类型">
                    <Option v-for="(item,index) in itemrole" :value="index" :key="item">{{ item }}</Option>
                </Select>
                 <Select v-show="formDate[5]&&formDate[5]>0" :disabled="index!=='add'" v-model="formDate[6]" @on-change="getBrand" style="width:160px;margin-right: 15px;" placeholder="请选择品牌">
                    <Option v-for="item in brand" :value="item.guid" :key="item.guid">{{ item.title }}</Option>
                </Select>
                 <Select v-show="formDate[6]" multiple :disabled="index!=='add'" v-model="categorylist" style="width:304px" placeholder="请选择类别">
      ...
      postSpecial(){
           if (this.formDate[0]&&this.formDate[1]&&this.formDate[2]&&this.formDate[3]&&this.formDate[4]&&this.imgmodels) {
              ...
              if(this.index==="add"){
                ...
                brandtitle,categoryname:JSON.stringify(categoryname),categoryguid:JSON.stringify(this.categorylist),picture:this.imgmodels,isopenutl:this.isopen}
              } else{
                  url = '/admin/common/coupon.ashx?action=edit';
                  addparme = {id:this.data1[this.index].id,title:this.formDate[0],Price:this.formDate[1],count:this.formDate[2],limitcount:this.formDate[3],stardate:this.formDate[4][0],enddate:this.formDate[4][1],picture:this.imgmodels}
              }  
